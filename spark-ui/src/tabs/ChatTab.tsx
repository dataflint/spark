...
import { Alert, FormControl, IconButton, Input, InputAdornment, InputLabel, Snackbar } from "@mui/material";
import * as React from "react";
import { useAppDispatch, useAppSelector } from "../Hooks";
import { SparkSQLStore } from "../interfaces/AppStore";
import { addMessage, setApiKey, setInputText, setIsTyping } from "../reducers/ChatSlice";
...
function formatToJsonKeyValue(sql: SparkSQLStore): string {
    ...
    sql.sqls.forEach((sql) => {
        ...
        const nodeNames = sql.nodes.filter(node => node.type !== "output").map((node: any) => node.nodeName).join(', ');
...
export default function ChatTab() {
    ...
    async function processMessageToChatGPT(chatMessages: MessageModel[]) {
        const apiMessages = chatMessages.map((messageObject) => {
            const role = messageObject.sender === "Flint Assistant" ? "assistant" : "user";
        ...
        const apiRequestBody = {
            ...
            "messages": [
                { role: "system", content: `You are an AI assistant for helping with analyzing an Apache Spark job. The resulted answers is presented as HTML so please, instead of ** for bold use <strong> and so on` },
                { role: "system", content: `here is a json that describe metrics of the spark job run: ${JSON.stringify(status)}` },
                { role: "system", content: `here is a key: value representations that describe the SQL queries of the spark job run: ${sql ? formatToJsonKeyValue(sql) : "no sql data"}` },
                ...apiMessages,
            ],
        };
                    ...
                    <MainContainer>
                        <ChatContainer>
                            <MessageList
                                ...
                                typingIndicator={isTyping ? <TypingIndicator content="Flint Assistant is typing" /> : null}
                            >
                                {messages.map((message, i) => {
                                    return <Message key={i} model={message} />
                                })}
                            </MessageList>
                            <MessageInput value={inputText}
                                onChange={(innerHtml, text) => dispatch(setInputText({ inputText: text }))} disabled={apiKey === undefined}
                                placeholder={apiKey === undefined ? "Set the API key to start chatting" : "Ask Flint Assistant anything..."}
                                attachButton={false}
            ...
            <div style={{
                ...
                <FormControl sx={{ width: '550px', margin: "20px" }} variant="standard">
                    ...
                    <Input
                        ...
                        onChange={event => dispatch(setApiKey({ apiKey: event.target.value }))}
