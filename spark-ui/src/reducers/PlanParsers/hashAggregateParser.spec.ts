...
describe('parseHashAggregate', () => {
    const testCases = [
        {
            input: "HashAggregate(keys=[sr_customer_sk#1188, sr_store_sk#1192], functions=[partial_sum(UnscaledValue(sr_return_amt#1196))])",
            expected: {
        ...
        {
            input: "HashAggregate(keys=[i_item_id#897, i_item_desc#900, i_category#908, i_class#906, i_current_price#901], functions=[partial_sum(UnscaledValue(ws_ext_sales_price#1263))])",
            expected: {
        ...
        {
            input: "HashAggregate(keys=[], functions=[sum(UnscaledValue(ws_ext_ship_cost#96446)), sum(UnscaledValue(ws_net_profit#96451)), count(distinct ws_order_number#96435L)])",
            expected: {
                keys: [],
                functions: ["sum(UnscaledValue(ws_ext_ship_cost))", "sum(UnscaledValue(ws_net_profit))", "count(distinct ws_order_numberL)"],
