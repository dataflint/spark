...
describe("parseTakeOrderedAndProject", () => {
  const testCases: {
    input: string;
    expected: ParsedTakeOrderedAndProjectPlan;
  }[] = [
    {
      input:
        "TakeOrderedAndProject(limit=100, orderBy=[s_store_name#1001 ASC NULLS FIRST], output=[s_store_name#1001,sum(ss_net_profit)#26850])",
      expected: {
