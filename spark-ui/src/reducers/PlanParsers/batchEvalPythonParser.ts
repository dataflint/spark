...
export function parseBatchEvalPython(input: string): ParsedBatchEvalPythonPlan {
    ...
    // Pattern: BatchEvalPython [first_list], [second_list] - with flexible whitespace
    const regex = /^BatchEvalPython\s*\[\s*(.*?)\s*\]\s*,\s*\[\s*(.*?)\s*\]$/;
    const match = cleanedInput.match(regex);

    if (!match) {
        throw new Error("Invalid BatchEvalPython input format");
    }
