...
export function reduceSQLInputOutputAlerts(sql: SparkSQLStore, alerts: Alerts) {
    sql.sqls.forEach((sql) => {
        sql.nodes.forEach((node) => {
            const filesReadMetric = parseFloat(
                node.metrics.find((metric) => metric.name === "files read")?.value?.replaceAll(",", "") ??
                "0",
            );
    ...
    2. Decrease number of executors, so each executor will read more small files and reduce the resource overhead
            ...
            const fileWrittenMetric = parseFloat(
                node.metrics.find((metric) => metric.name === "files written")?.value?.replaceAll(",", "") ??
                "0",
            );
            ...
            if (node.type === "output" && node.parsedPlan?.type === "WriteToHDFS" && fileWrittenMetric && bytesWrittenMetric) {
                ...
                if (
                    ...
                    const isPartitioned = node.parsedPlan.plan.partitionKeys !== undefined;
                    if (isPartitioned && node.parsedPlan.plan.partitionKeys !== undefined) {
                        const partitionsWritten = parseFloat(
                            node.metrics.find((metric) => metric.name === "partitions written")?.value?.replaceAll(",", "") ??
                            "0",
                        );
                        const expectedAvgFileSize = partitionsWritten !== 0 ? humanFileSize(bytesWrittenMetric / partitionsWritten) : 0;
                        const filesPerPartition = partitionsWritten !== 0 ? fileWrittenMetric / partitionsWritten : 0;
                        const partitionKeysText = node.parsedPlan.plan.partitionKeys.map(key => `"${key}"`).join(",")
                        const partitionKeysStringParam = node.parsedPlan.plan.partitionKeys.map(key => `"${key}"`).join(",")
                        alerts.push({
        ...
        2. Choose different partition key${partitionKeysText.length === 1 ? "" : "s"} instead of ${partitionKeysText} with lower cardinality`,
                            type: "warning",
                            source: {
                                type: "sql",
                                sqlId: sql.id,
                        ...
                        const idealFileCount = Math.ceil(bytesWrittenMetric / IDEAL_FILE_SIZE);
                        const fileSizeAfterRepartition = humanFileSize(bytesWrittenMetric / idealFileCount);
                        alerts.push({
                            id: `writeSmallFiles_${sql.id}_${node.nodeId}_${avgFileSizeString}`,
