...
export function reduceSQLInputOutputAlerts(sql: SparkSQLStore, alerts: Alerts) {
    sql.sqls.forEach(sql => {
        sql.nodes.forEach(node => {
            const filesReadMetric = parseFloat(node.metrics.find(metric => metric.name === "files read")?.value ?? "0");
            const bytesReadMetric = parseFloat(node.metrics.find(metric => metric.name === "bytes read")?.value ?? "0");
            ...
            if (node.type === 'input' &&
                ...
                if (avgFileSize < BYTE_READ_TOO_SMALL_THRESHOLD && filesReadMetric > FILES_READ_TOO_HIGH_THREASHOLD) {
                    alerts.push({
            ...
            const fileWrittenMetric = parseFloat(node.metrics.find(metric => metric.name === "files written")?.value ?? "0");
            const bytesWrittenMetric = parseFloat(node.metrics.find(metric => metric.name === "bytes written")?.value ?? "0");
