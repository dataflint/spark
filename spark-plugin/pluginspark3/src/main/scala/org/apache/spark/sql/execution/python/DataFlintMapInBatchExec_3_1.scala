/*
 * DataFlint instrumented MapInBatchExec for Spark 3.1.x
 *
 * Spark 3.1 characteristics:
 ...
 *   - Added simplifiedTraceback on ArrowPythonRunner (SPARK-33407)
 */
...
/**
 * DataFlint instrumented version of MapInBatchExec for Spark 3.1.x.
 * Adds a duration metric to measure time spent in Python UDF execution.
 *
 * Uses reflection to construct ArrowPythonRunner since the 3.1 constructor
 * (6-arg) differs from the 3.5 constructor (9-arg) that this code compiles against.
 */
trait DataFlintMapInBatchExec_3_1 extends SparkPlan with Logging {
  ...
  // No PythonSQLMetrics in 3.1 — only our custom duration metric
  ...
  // $default$3() method which doesn't exist in 3.1 at runtime.
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    logInfo("DataFlint MapInBatchExec (Spark 3.1) doExecute is running")
    ...
    child.execute().mapPartitionsInternal { inputIter =>
      ...
      // Construct confMap manually (replicates ArrowUtils.getPythonRunnerConfMap for 3.1)
      ...
      // Use reflection: Spark 3.1 ArrowPythonRunner has a 6-arg constructor
      val runnerClass = classOf[ArrowPythonRunner]
      val ctor = runnerClass.getConstructors
        .find(_.getParameterCount == 6)
        .getOrElse(throw new RuntimeException(
          "Cannot find ArrowPythonRunner 6-arg constructor. Expected Spark 3.1.x runtime."))
...
/**
 * DataFlint instrumented MapInPandasExec for Spark 3.1.x.
 * Replaces the original MapInPandasExec in the physical plan.
 *
 * Note: mapInArrow is not available in Spark 3.1 (introduced in 3.3 via SPARK-37227).
 */
case class DataFlintMapInPandasExec_3_1(
    func: Expression,
    output: Seq[Attribute],
    child: SparkPlan)
  extends DataFlintMapInBatchExec_3_1 with Logging {

  override def nodeName: String = "DataFlintMapInPandas"

  logInfo("DataFlint MapInPandas (Spark 3.1) is connected")
