...
import org.apache.iceberg.metrics.{CommitMetrics, CommitReport, MetricsReport, MetricsReporter}
  ...
  def report(report: MetricsReport): Unit = {
    ...
    report match {
      case commitMetrics: CommitReport =>
        println(s"commitMetrics for sql query ${SparkSession.active.sparkContext.getLocalProperty("spark.sql.execution.id")}")
        println(commitMetrics)
      case _ =>
    }
...
object IcebergExample extends App{
  val spark = SparkSession
    ...
    .config("spark.dataflint.iceberg.enabled", true)
