-- TPC-DS Query 4
-- Find customers who spend more in stores, web, and catalog in one year than the next.
WITH year_total AS (
  SELECT
  ...
  SELECT
    ...
    c_email_address AS customer_email_address,
    d_year AS dyear,
    SUM(cs_ext_list_price - cs_ext_discount_amt) AS year_total_amt,
    'c' AS sale_type
  FROM customer
...
WHERE t_s_firstyear.year_total_amt > 0
  AND t_c_firstyear.year_total_amt > 0
  AND t_w_firstyear.year_total_amt > 0
  AND CASE WHEN t_s_firstyear.year_total_amt > 0
    THEN t_s_secyear.year_total_amt / t_s_firstyear.year_total_amt
    ELSE NULL END
    > CASE WHEN t_c_firstyear.year_total_amt > 0
    THEN t_c_secyear.year_total_amt / t_c_firstyear.year_total_amt
    ELSE NULL END
  AND CASE WHEN t_c_firstyear.year_total_amt > 0
    THEN t_c_secyear.year_total_amt / t_c_firstyear.year_total_amt
    ELSE NULL END
    > CASE WHEN t_w_firstyear.year_total_amt > 0
